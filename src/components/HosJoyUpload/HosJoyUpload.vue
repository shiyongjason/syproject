            ...
            <el-dialog title='图片裁剪' width='1000px' center :visible.sync='croppervisible' append-to-body :close-on-press-escape='false' :close-on-click-modal='false' :before-close="handleCropClose">
                <div style='widrh:600px;height:450px'>
                    <component
                        ...
                        :outputType="cropperOutputType||outputType"
                        :info="true"
                        :full="full"
                        :canMove="canMove"
                ...
                <span slot='footer' class='dialog-footer'>
                    <el-button @click="handleCropClose">取 消</el-button>
...
export default {
    ...
    props: {
        ...
        // 以下是裁剪属性 https://github.com/xyxiao001/vue-cropper
        outputSize: { type: Number, default: 1 }, // 裁剪生成图片的质量 0.1 - 1
        outputType: { type: String, default: 'jpeg' }, // 裁剪生成图片的格式 jpeg || png || webp,,,,,jpg (jpg 需要传入jpeg)
        canScale: { type: Boolean, default: true }, // 图片是否允许滚轮缩放
        autoCrop: { type: Boolean, default: false }, // 是否默认生成截图框
        autoCropWidth: String, // 默认值容器的80% 默认生成截图框宽度 0~max
        autoCropHeight: String, // 默认值容器的80% 默认生成截图框高度0~max
        fixed: Boolean, // 是否开启截图框宽高固定比例 默认true
        fixedNumber: Array, // 截图框的宽高比例 默认[1, 1]
        full: Boolean, // 是否输出原图比例的截图 默认false
        fixedBox: { type: Boolean, default: false }, // 固定截图框大小 不允许改变 false
        canMove: { type: Boolean, default: true }, // 上传图片是否可以移动 true
        canMoveBox: { type: Boolean, default: true }, // 截图框能否拖动 true true | false
        original: Boolean, // 上传图片按照原始比例渲染 false
        centerBox: Boolean, // 截图框是否被限制在图片里面 false
        high: Boolean, // 是否按照设备的dpr 输出等比例图片 true
        infoTrue: Boolean, // true 为展示真实输出图片宽高 false 展示看到的截图框宽高 false
        maxImgSize: { type: Number, default: 10000 }, // 限制图片最大宽度和高度 2000 0-max
        enlarge: Number, // 图片根据截图框输出比例倍数 1 0-max(建议不要太大不然会卡死的呢)
        mode: String // 图片默认渲染方式 contain contain , cover, 100px auto, 100% auto
    ...
    methods: {
        ...
        beforeAvatarUpload (file) {
            ...
            if (this.showCrop) {
                ...
                if (file.name) {
                    if (file.name.slice(file.name.lastIndexOf('.')).toLowerCase() == '.png') {
                        this.cropperOutputType = 'png'
                    }
                }
                this.croppervisible = true
                // MARK 如果是裁剪的要return false 阻止进行默认的一下步上传
                return false
            }
        },
        startCrop () {
    ...
    mounted () {
        ...
        if (this.showCrop) {
            this.cropper = () => import('./crop.vue')
        }
    }
    // beforeRouteLeave (to, from, next) {
    //     console.log('beforeRouteLeave', 111)
    //     next()
    // }
    // beforeDestroy () {
    //     console.log('beforeDestroy', 111)
    // }
}
</script>
