<template>
        <div class="agel-form-inline">
            <template v-for="(row,rowIndex) in dynamicData">
                ...
                <agel-form-item
                    v-for="(item,colIndex) in agItems"
                    :key="row._key_+item.prop"
                    v-show="item.show"
...
<script>
import itemsMinxin from './itemsMixin'
import agelFormItem from './agel-form-item.vue'
export default {
    // 行内表单
    name: 'hosjoy-form-inline',
    mixins: [itemsMinxin],
    inheritAttrs: false,
    components: {
        agelFormItem
    ...
    mounted () {
        console.log('log::::::this.agItems', this.agItems)
    }
}
</script>
