<template>
    <div class="page-body">
        ...
        <drawerCom :drawer=drawer @backEvent='restDrawer' :merchantCode='companyCode'></drawerCom>
    </div>
</template>
<script>
    ...
    methods: {
        ...
        onOperate (val) {
            this.$confirm(val.isEnabled == 0 ? '是否确认启用该商家的账号?' : '是否确认禁用该商家的账号？禁用后商家将无法使用相关功能，你还要继续吗？', '提示', {
                ...
                await changeState({ merchantCode: val.companyCode, status: val.isEnabled == 0 ? 1 : 0 })
                this.$message({
