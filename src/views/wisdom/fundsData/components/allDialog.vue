    ...
    <div>
        <el-dialog :title="title" :visible.sync="dialogVisible" width="60%" :close-on-click-modal='false' :before-close="handleClose">
            <div class="dialog_wrap">
                ...
                <el-descriptions v-if="false" title="流贷借款账目">
                    <!-- 流贷 -->
                ...
                <el-descriptions v-if="false" title="流贷还款账目">
                    <!-- 流贷 -->
                ...
                <el-descriptions v-if="false" title="逾期账目">
                    <!-- 流贷 -->
                    ...
                    <el-descriptions-item label="手动调息" v-if="hosAuthCheck(WISDOM_FLOWTOBORROW_SHOW_LINE)">{{(dialogDetail.paymentStatic_normalInterestPranayamaTotal || dialogDetail.paymentStatic_graceInterestPranayamaTotal || dialogDetail.paymentStatic_overDueInterestPranayamaTotal) ? '已调息' : '-'}}
                        <i class='el-icon-edit pointer' @click="getRespAccountRepaymentPlanData(dialogDetail, `${product}-流贷还款信息维护（${dialogDetail.account_standingBookNo} ${dialogDetail.account_loanCompanyName}）`, dialogDetail.account_id, 'regulatingBreathingDialogVisible')"></i>
                    </el-descriptions-item>
                ...
                <el-descriptions title="敞口还款账目总计">
                ...
                <el-descriptions title="敞口还款账目明细">
                    <!-- 敞口 -->
                    ...
                    <el-descriptions-item label="应缴逾期罚息" v-if="hosAuthCheck(WISDOM_EXPOSURE_SHOW_LINE)">{{dialogDetail.planList_2_graceInterestPaid |moneyFormat}}</el-descriptions-item>
                    ...
                    <el-descriptions-item label="手动调息" v-if="hosAuthCheck(WISDOM_EXPOSURE_FUNDSDATA_UPDATA)">
                        {{(dialogDetail.paymentStatic_normalInterestPranayamaTotal || dialogDetail.paymentStatic_graceInterestPranayamaTotal || dialogDetail.paymentStatic_overDueInterestPranayamaTotal) ? '已调息' : '-'}}<i class='el-icon-edit pointer'
