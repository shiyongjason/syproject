<template>
    <div class="wrapper">
        ...
        <el-table :data="tableData" border height="573" v-loading="loading" style="width: 100%">
            ...
            <el-table-column prop="subsectionName" width="100" label="所属分部" v-if="selectTh.indexOf('所属分部')>-1">
                <template slot-scope="scope">
                    <span v-if="scope.row.subsectionName === null">-</span>
                    <span v-else>{{scope.row.subsectionName}}</span>
                </template>
            </el-table-column>
            <el-table-column prop="cityName" label="城市" v-if="selectTh.indexOf('城市')>-1">
                <template slot-scope="scope">
                    <span v-if="scope.row.cityName === null">-</span>
                    <span v-else>{{scope.row.cityName}}</span>
                </template>
            </el-table-column>
            <el-table-column prop="onlineTime" width="120" label="上线时间" v-if="selectTh.indexOf('上线时间')>-1">
                <template slot-scope="scope">
            ...
            <el-table-column prop="incremental" width="100" :formatter="incremental" label="增量/存量" v-if="selectTh.indexOf('增量/存量')>-1">
            </el-table-column>
            <el-table-column prop="subsectionRank" label="分部排名" v-if="selectTh.indexOf('分部排名')>-1">
                <template slot-scope="scope">
                    <span v-if="scope.row.subsectionRank === null">-</span>
                    <span v-else>{{scope.row.subsectionRank}}</span>
                </template>
            </el-table-column>
            <el-table-column prop="countryRank" label="全国排名" v-if="selectTh.indexOf('全国排名')>-1">
                <template slot-scope="scope">
                    <span v-if="scope.row.countryRank === null">-</span>
                    <span v-else>{{scope.row.countryRank}}</span>
                </template>
            </el-table-column>
            <el-table-column label="达成" v-if="selectTh.indexOf('达成')>-1">
            ...
            <el-table-column label="同比" v-if="selectTh.indexOf('同比')>-1">
        ...
        <div class="page clearfix" style="text-align: center;margin-top: 20px">
            <el-pagination class="el-page" background @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="paginationData.pageNumber" layout="total, sizes, prev, pager, next, jumper" :page-sizes="[10, 20, 30, 40, 50]" :total="paginationData.totalElements">
            </el-pagination>
        </div>
    </div>
</template>
