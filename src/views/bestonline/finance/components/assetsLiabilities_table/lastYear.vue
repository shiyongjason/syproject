<template>
    <div class="yearTable">
        <table class="item-wrapper no_right_border">
            ...
            <tbody v-if="form.assetsLiabilities.assetListL">
                <tr v-for="(item,index) in form.assetsLiabilities.assetListL" :key="index">
                    ...
                    <template v-else>
                        ...
                        <td>
                            <template v-if="item.typeName === '应收账款' || item.typeName === '预付款项' || item.typeName === '其他应收款' || item.typeName === '存货' || item.typeName === '待摊费用' || item.typeName === '流动资产合计' || item.typeName === '非流动资产合计' || item.typeName === '资产总计'">
                                <el-form-item label-width="0" :prop="`assetsLiabilities.assetListL[${index}].endOrCurrent`" :rules="rules.endOrCurrent">
                        ...
                        <td>
                            <template v-if="item.typeName === '应收账款' || item.typeName === '预付款项' || item.typeName === '其他应收款' || item.typeName === '存货' || item.typeName === '待摊费用' || item.typeName === '流动资产合计' || item.typeName === '非流动资产合计' || item.typeName === '资产总计'">
                                <el-form-item label-width="0" :prop="`assetsLiabilities.assetListL[${index}].beginOrPrior`" :rules="rules.beginOrPrior">
        ...
        <table class="item-wrapper">
            ...
            <tbody v-if="form.assetsLiabilities.liabilitiesListL">
                <tr v-for="(item,index) in form.assetsLiabilities.liabilitiesListL" :key="index">
                    ...
                    <template v-else>
                        ...
                        <td>
                            <template v-if="item.typeName === '短期借款' || item.typeName === '流动负债合计' || item.typeName === '非流动负债合计' || item.typeName === '负债合计'">
                                <el-form-item label-width="0" :prop="`assetsLiabilities.liabilitiesListL[${index}].beginOrPrior`" :rules="rules.beginOrPrior">
                                    <el-input v-model="item.beginOrPrior" placeholder="请输入内容" maxlength="25">
                                        <template slot="suffix">万</template>
                                    </el-input>
                                </el-form-item>
