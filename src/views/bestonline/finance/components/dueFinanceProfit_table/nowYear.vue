<template>
    <div class="yearTable">
        <table class="item-wrapper">
            ...
            <tbody v-if="form.dueFinanceProfit.contentListT">
                <tr v-for="(item,index) in form.dueFinanceProfit.contentListT" :key="index">
                    ...
                    <template v-else>
                        ...
                        <td>
                            ...
                            <template v-else>
                                <el-form-item label-width="0" :prop="`dueFinanceProfit.contentListT[${index}].endOrCurrent`" :rules="rules.endOrCurrentNORequired">
                                    <el-input v-model="item.endOrCurrent" placeholder="请输入内容" maxlength="25">
                                        <template slot="suffix">万</template>
                                    </el-input>
                                </el-form-item>
                        ...
                        <td>
                            ...
                            <template v-else>
                                <el-form-item label-width="0" :prop="`dueFinanceProfit.contentListT[${index}].beginOrPrior`" :rules="rules.beginOrPriorNORequired">
                                    <el-input v-model="item.beginOrPrior" placeholder="请输入内容" maxlength="25">
                                        <template slot="suffix">万</template>
                                    </el-input>
                                </el-form-item>
...
export default {
    data () {
        return {
            rules: {
                endOrCurrent: [
                    { required: true, message: '请填写本年度累计金额（本年度）', trigger: 'blur' },
                ...
                beginOrPrior: [
                    { required: true, message: '请填写上年末度累计金额（本年度）', trigger: 'blur' },
                    { validator: MoneyOrConnectorMinus }
                ],
